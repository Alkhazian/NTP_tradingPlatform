...
class SPXRangeStrategy(SPXBaseStrategy):
    ...
    def _on_fill_timeout(self, event):
        ...
        # IB can report spread fills via individual LEG events without updating
        # the parent spread order's filled_qty.  Use the actual position (via
        # legs in cache) as the source of truth, falling back to order.filled_qty.
        order_filled_qty = float(order.filled_qty)
        actual_filled_qty = abs(self.get_effective_spread_quantity())
        filled_qty = max(actual_filled_qty, order_filled_qty)
        total_qty = float(order.quantity)
        unfilled_qty = total_qty - filled_qty

        # SCENARIO 0: All contracts already filled (legs filled but spread
        # order status still PARTIALLY_FILLED).  Nothing to cancel or update.
        if filled_qty >= total_qty:
            self.logger.info(
                f"⏱️ Fill timeout fired but all {total_qty:.0f} contracts already filled "
                f"(effective_qty={actual_filled_qty:.0f}, order.filled_qty={order_filled_qty:.0f}) | "
                f"No action needed | Order: {self._entry_order_id}",
                extra={
                    "extra": {
                        "event_type": "fill_timeout_already_filled",
                        "order_id": str(self._entry_order_id),
                        "effective_qty": actual_filled_qty,
    ...
    def on_order_filled_safe(self, event):
        ...
        # IB may report all fills via individual LEG events without ever
        # setting the parent spread order status to FILLED.  Therefore we
        # also check the actual position size via get_effective_spread_quantity().
        if self._entry_order_id:
            order = self.cache.order(self._entry_order_id)
            total_qty = float(order.quantity) if order else 0
            actual_qty = abs(self.get_effective_spread_quantity())
            order_fully_filled = order and order.status == OrderStatus.FILLED
            legs_fully_filled = actual_qty >= total_qty and total_qty > 0

            if order_fully_filled or legs_fully_filled:
                try:
                    self.clock.cancel_timer(f"{self.id}_fill_timeout")
                except Exception:
                    pass
                ...
                self.logger.info(
                    ...
                    extra={
                        "extra": {
                            ...
                            "total_qty": total_qty,
                            "order_status": order.status.name if order else "N/A"
                        }
                    }
                )
